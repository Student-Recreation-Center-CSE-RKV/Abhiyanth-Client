...
import { addEvent, getAllEvents, deleteEvent, updateEvent } from "../../api/events.js";
...
import { Box, Grid, Card, CardContent, Typography, Button, TextField, CardMedia, CardActions } from "@mui/material";
...
const ManageEventsByAdmin = () => {
 ...
 const fetchEvents = async() =>{
      ...
      setEvents([...data.completed, ...data.ongoing, ...data.upcoming]);
      setFilteredEvents([...data.completed, ...data.ongoing, ...data.upcoming]);
    ...
    <Box sx={{ padding: "2rem" }}>
      ...
      <Grid container spacing={2} mb={4}>
        {['completed', 'live', 'upcoming'].map((status, idx) => (
          <Grid item xs={12} sm={4} key={status}>
            <Card sx={{ width: "247.3px", cursor: 'pointer' }} onClick={() => console.log(`${status} clicked`)}>
              <CardContent>
                <Typography variant="h5" component="div" textAlign="center">
                  {status.charAt(0).toUpperCase() + status.slice(1)}
                </Typography>
                <Typography variant="h6" color="text.secondary" textAlign="center">
                  {events.filter((event) => event.status === status).length} Events
                </Typography>
              </CardContent>
            </Card>
          </Grid>
      ...
      <Grid container spacing={2}>
        {filteredEvents.map((event) =>(
            ...
            <Card sx={{ height: "100%", display: "flex", flexDirection: "column" , width:"247.3px" }}>
                  <CardMedia
                    component="img"
                    height="140"
                    image={deptImage}
